...
**Status:** dev.5 scope closure is in progress as forward-only commits on top of frozen dev.4. Radar runtime, Cilium/Hubble Network Live, executable native diagnostics, Operator Center UI and trusted Kubernetes day-2 execution are already committed/pushed with exact-SHA `validate` CI success through `4fdbee7800f65e97509a997fac3bca1e95d6dbce`. The current uncommitted slice adds active unified cluster verification while preserving explicit SKIP/CONTRACT_ONLY states where host/provider/etcd/agent runtime evidence is unavailable. Do not amend, move, or recreate dev.4. Dev.5 must still close provider/Cluster Factory and air-gap runtime gaps, provider-coupled verification extensions, and the final audit before `v0.5.11-dev.5` is created. Production image publication remains GitHub Actions -> Docker Hub.
