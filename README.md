...
- digest-pinned OCI/Helm/package/Git-release artifact plans plus executable controlled file/HTTPS-to-file blob sync and OCI-image plus typed Helm OCI registry-to-registry synchronization with source/destination digest verification, exact ClusterBlueprint artifact bindings with verified offline manifest resolution, and READY-manifest binding into governed provisioning plans/provider-job requests
