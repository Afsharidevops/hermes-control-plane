...
> Dev.5 scope closure now includes Radar runtime, Cilium/Hubble Network Live, executable native diagnostics, a full Hermes-native Operator Center UI scope contract, bounded trusted-Kubernetes day-2 execution, active unified cluster verification, and the Batch C trusted Redfish infrastructure runtime for inventory/power/boot/virtual-media/BIOS/firmware/storage plus bounded Secure Boot/SR-IOV/IOMMU/persistent boot-order desired-state execution. A read-only Proxmox capacity collector is also implemented as a disabled-by-default Node Agent worker, returning only host utilization evidence from authenticated PVE API calls. UI state is reported separately from provider/runtime completion.
...
> **0.5.11-dev.5 scope closure in progress:** dev.5 remains forward-only from frozen `v0.5.11-dev.4`. The completed runtime includes Radar, Cilium/Hubble, native diagnostics, Operator Center, bounded Kubernetes day-2, unified verification, Argo CD/Cilium/Velero operations, digest-pinned blob/OCI/Helm/Git/Ansible artifact mirroring, ClusterBlueprint dependency resolution and READY offline-manifest binding, plus Batch A signed APT/RPM/Python repository snapshots and development image-publication suppression. Merged Batch B adds a disabled-by-default trusted provider worker on the existing Node Agent image for exact-ticket-bound offline Kubespray v2.28.1, K3s and RKE2 execution, with deterministic offline reference consumption, private staged SSH profiles, fixed playbook/command vectors, bounded worker lifecycle, Kubernetes upgrades, certificate rotation, existing-host maintenance, and K3s/RKE2 embedded-etcd snapshot/restore and existing-host DR with active verification. Batch B deliberately fails closed for provider-capacity creation/destruction, true cluster decommission/infrastructure scale/template cloning, Kubespray direct-etcd recovery and provider-specific real-target collectors. Batch C has now begun locally with a disabled-by-default trusted Redfish `inventory.refresh`/`power.set`/`boot.set`/`virtual-media.insert`/`virtual-media.eject`/`bios.apply`/`firmware.apply` worker path plus constrained IPMI LAN+ power/boot fallback and private-offline PXE/iPXE unattended provisioning and allowlisted Redfish disk/RAID desired-state runtime. These paths use mounted worker credentials, active preview/diff, exact preview-state/artifact drift rejection, signed one-time execution tickets and post-change verification; management/provisioning networking, switch/network, VMware vSphere/VMware Workstation/OpenStack/AWS/Azure/GCP capacity runtime and matching real-target collectors remain open. A read-only Proxmox capacity collector is now implemented (disabled by default) returning host utilization evidence from authenticated PVE API calls; it is not a placement guarantee. VMware Workstation remains contract-only for capacity — its API does not provide verifiable free host capacity. Normal `dev/**` pushes/PRs validate without publishing all seven images. `v0.5.11-dev.5` is not yet release-complete or tagged.
...
- Proxmox VM inventory collector (`vm.inventory.refresh`): a separate, disabled-by-default read-only collector pinned to `pve-vm-inventory-v1`, independent of the capacity collector's `pve-capacity-v1` pin (an operator wanting both registers two Proxmox provider entries). A `LIVE` observation requires exactly two successful authenticated PVE reads — `/cluster/resources?type=node` then `/cluster/resources?type=vm` — and every allowlisted node must appear in the node response before the VM read is attempted, so an empty VM result is truthful coverage rather than an inferred one. Records are limited to `{vm_id, node, type, power_state, template}`, deterministically sorted, bounded to 512, and filtered to allowlisted nodes; names, IPs, MACs, tags, disk/storage, owner/pool, raw PVE bodies, endpoints and credentials are never returned. This is identity and power-state observation only — not capacity, placement or lifecycle proof — and all Proxmox VM mutation operations remain contract-only. Local/mock test evidence only; no real Proxmox target has returned a `LIVE` VM inventory observation.
