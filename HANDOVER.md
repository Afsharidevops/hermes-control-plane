...
**Status:** dev.5 scope closure is in progress as new commits on top of frozen dev.4. Radar runtime, Cilium/Hubble Network Live and executable Hermes-native diagnostics are already committed/pushed with exact-SHA `validate` CI success. The current uncommitted slice closes the Hermes-native Operator Center UI surface while preserving explicit PARTIAL/CONTRACT_ONLY runtime states for unfinished executors. Do not amend, move, or recreate dev.4. Dev.5 must complete the remaining runtime audit gaps, pass full validation and exact-SHA branch CI before `v0.5.11-dev.5` is created. Production image publication remains GitHub Actions -> Docker Hub.
