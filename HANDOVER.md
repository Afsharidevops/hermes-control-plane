...
**Stable base:** `v0.5.11` — frozen / do not recreate
...
**Released v0.5.11 boundary:** `237900c2a0d37f0d46383a67d3aea7f99e341a96`, tag `v0.5.11`
**Status:** `v0.5.11` is released. Dev.5 scope closure (forward-only work on top of frozen dev.4) completed with the addition of a disabled-by-default, worker-only Proxmox QEMU VM mutation runtime (eight governed operations) alongside the existing read-only Proxmox capacity/VM-inventory collectors, plus the operator validation runbook. `./validate.sh` passed locally, exact-SHA `validate` CI succeeded at `237900c`, and the tag was pushed straight to `v0.5.11` (no separate `v0.5.11-dev.5` tag). Docker Hub images are published as `:0.5.11`/`:latest`; see the GitHub Release at `v0.5.11`. VMware Workstation/vSphere, OpenStack, AWS, Azure, GCP mutation/capacity runtime, and capacity-backed Cluster Factory lifecycle (C11) remain explicitly deferred to a future release — Cluster Factory continues to require existing, pre-registered, preflight-PASS servers. Direct etcd snapshot/restore, full-cluster/provider DR, apt/yum/dnf/Python repository protocol breadth beyond what shipped, broader Ansible Galaxy catalog/role handling, and provider-worker offline-reference consumption/rewrite remain open for a future release. Do not amend, move, or recreate dev.4 or the released `v0.5.11` tag. Production image publication remains GitHub Actions -> Docker Hub. Confirm with the operator whether draft PR `#2` should be merged/closed to match the tagged release.
...
Per the user-requested provider scope decision, `proxmox` has a narrow disabled-by-default, worker-only **QEMU mutation runtime** (`pve-8.2` / `pve-vm-runtime-v1`) for exactly `vm.create`, `vm.clone`, `vm.update`, `vm.delete`, `vm.power`, `network.attach`, `snapshot.create`, and `snapshot.restore`. Its strict capability snapshots, active preview/current-hash drift checks, one-time signed tickets, fixed HTTPS PVE transport, bounded task polling and active readback verification are implemented locally; `vm.delete` and `snapshot.restore` are CRITICAL/two-approval, and the other six actions are HIGH. This is not capacity-backed Cluster Factory lifecycle and has no real-provider claim until an operator provides sanitized disposable-target evidence at the exact pushed SHA. `vmware-workstation` remains CONTRACT_ONLY. VMware vSphere, OpenStack, AWS, Azure, GCP, and capacity-backed Cluster Factory lifecycle (C11) are explicitly deferred; Cluster Factory remains existing-host/pre-registered-server only.
