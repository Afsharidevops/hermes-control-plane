...
NATIVE_DIAGNOSTICS: list[dict[str, Any]] = [
    {"id": "nodes.not-ready", "source": "kubernetes", "mode": "read", "description": "Detect NotReady nodes and pressure conditions."},
    {"id": "workloads.unhealthy", "source": "kubernetes", "mode": "read", "description": "Detect unavailable Deployments, StatefulSets and DaemonSets."},
    {"id": "pods.restart-hotspots", "source": "kubernetes", "mode": "read", "description": "Summarize repeated container restarts without reading Secret values."},
    {"id": "events.warning-summary", "source": "radar", "mode": "read", "description": "Aggregate recent warning events by reason and namespace."},
    {"id": "network.policy-drops", "source": "hubble", "mode": "read-aggregated", "description": "Summarize authorized/redacted Hubble policy drops."},
    {"id": "storage.pvc-pending", "source": "kubernetes", "mode": "read", "description": "Detect Pending PVCs and unavailable storage classes."},
    {"id": "certificates.expiry", "source": "kubernetes", "mode": "read", "description": "Inspect certificate metadata and expiry without exporting private keys."},
    {"id": "backup.recency", "source": "velero", "mode": "read", "description": "Report backup success/failure and recency."},
]
