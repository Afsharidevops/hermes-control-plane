- Smart Router synced to the 0.6.1 component line: add the multi-agent
  Orchestrator (planner, supervised step execution, approve/reject gate,
  reviewer verdict with an optional rollback suggestion, run history) to the
  Operations Center with `v60_agent_runs` / `v60_agent_run_steps` persistence,
  adopt the Content Console design language, expose
  `SMART_ROUTER_ORCHESTRATOR_APPROVAL_MODE`, `SMART_ROUTER_ORCHESTRATOR_PLANNER_TIER`
  and `SMART_ROUTER_ORCHESTRATOR_REVIEWER_TIER`, and keep buffered chat clients
  on the JSON contract when an upstream gateway streams by default. The image
  tag for this repository remains driven by `VERSION`.
