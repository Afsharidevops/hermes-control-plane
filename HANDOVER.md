...
**Status:** dev.5 scope closure is in progress as forward-only commits on top of frozen dev.4. Eleven slices are committed/pushed with exact-SHA `validate` success through the Velero Schedule runtime at `afef647fd7bbed4e82ea34ed7405f21fd30e6c6b` (validate run `32410004464`). This slice adds bounded digest-pinned OCI-image registry-to-registry synchronization with fixed Skopeo arguments, full multi-arch copy, preserved digests, registry allowlists and active source/destination manifest verification. Direct etcd snapshot/restore, full-cluster/provider DR and provider-backed lifecycle remain open. Do not amend, move, or recreate dev.4. Dev.5 must still close provider/Cluster Factory runtime gaps, remaining air-gap protocols, provider-coupled verification extensions, and the final audit before `v0.5.11-dev.5` is created. Production image publication remains GitHub Actions -> Docker Hub.
