...
[[ "$(cat VERSION)" == "0.5.11-dev.4" ]] || { echo "VERSION is not 0.5.11-dev.4" >&2; exit 2; }
...
Create/refresh a repository virtualenv, for example:
  python3 -m venv .venv-dev4
  .venv-dev4/bin/python -m pip install -U pip pytest
  for req in control-plane/requirements.txt credential-service/requirements.txt \
    ...
    [ ! -f "$req" ] || .venv-dev4/bin/python -m pip install -r "$req"
  done
...
"$PYTHON_BIN" scripts/acceptance/dev4-source-security-gate.py
"$PYTHON_BIN" scripts/acceptance/dev4-config-static-gate.py
...
if [[ "${HERMES_VALIDATE_LOCAL_IMAGES:-0}" == "1" ]]; then
  ...
  for context in control-plane credential-service router-gateway smart-router execution-broker kubernetes-broker node-agent; do
    docker build --pull=false -t "hermes-validation/${context}:0.5.11-dev.4" "$context"
  done
fi

echo "Hermes 0.5.11-dev.4 validation: PASS"
