...
**Status:** dev.5 scope closure is in progress as forward-only commits on top of frozen dev.4. Nine slices are committed/pushed with exact-SHA `validate` success through the one-shot Velero Backup runtime at `dce1730f294861f5cf53de7083803eb7a5f5e5e5`. This slice adds a separately governed CRITICAL Velero namespace-restore runtime with two distinct approvals, exact source-Backup/Restore preconditions, fixed non-destructive Restore CR creation and active terminal verification. Velero schedules, direct etcd snapshot/restore, full-cluster/provider DR and provider-backed lifecycle remain open. Do not amend, move, or recreate dev.4. Dev.5 must still close provider/Cluster Factory runtime gaps, remaining air-gap protocols, provider-coupled verification extensions, and the final audit before `v0.5.11-dev.5` is created. Production image publication remains GitHub Actions -> Docker Hub.
