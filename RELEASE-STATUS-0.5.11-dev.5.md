...
- current continuation: typed digest-pinned Ansible Galaxy collection archive synchronization with exact namespace/name/SemVer identity, MANIFEST.json -> FILES.json checksum binding, per-file SHA-256 verification, unsafe tar member rejection and no filesystem extraction; pending commit/push/exact-SHA CI
...
- remaining air-gap protocol closure: OS/Python/package-repository metadata sync, broader Ansible Galaxy catalog/standalone-role handling, full Git-repository/submodule mirroring where required, broader signature policy, provider-worker/provisioner-side offline reference consumption/rewrite, and generalized authenticated repository credential delivery
